...
module meeple_guitarist_cookie_cutter(cutter_scale, cutter_height, cutter_thickness){
  difference() {
    ...
    union () {
      translate ([25,-cutter_thickness+0.2,cutter_height/2]) rotate([90,0,0]) ciy_logo(logo_size = cutter_height/2, logo_height = cutter_thickness, logo_orientation = 0);
      translate ([55,-cutter_thickness+0.2,cutter_height/2+1]) rotate([90,0,0]) recycling_symbol("PLA", cutter_height/2, cutter_thickness, [0,0,0]);
    }
  }
}
