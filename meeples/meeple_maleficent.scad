...
// ************************************************************
// Model for cooking tools in the shape of a Maleficent Meeple:
// - cookie cutter
...
scale([1,1,1]){
  ...
  // "cookie_cutter": cookie cutter in the shape of a Maleficent meeple
  // "mold" : the positive shape to create a mold of a Maleficent meeple
  ...
  if (export == "cookie_cutter") {
    meeple_maleficent_cookie_cutter(cutter_scale, cutter_height, cutter_thickness, cutter_color);
    
  }
  else if (export == "cookie_petg") {
    meeple_maleficent_cookie_cutter(cutter_scale, cutter_height, cutter_thickness, cutter_color, "PETG");
    
  }
  else if (export == "mold") {
    rotate([0,0,0]) scale([mold_scale,mold_scale,1]) {
      meeple_maleficent_mold(mold_height, mold_height*mold_rounding_factor, mold_fillet_steps, mold_color);
    }
    ...
    $fn = 30;
    meeple_maleficent_cookie_cutter(cutter_scale, cutter_height, cutter_thickness);
  }
...
module meeple_maleficent_shape(){
    import(file = "meeple_maleficent.dxf");
}

module meeple_maleficent_mold_shape(){
    import(file = "meeple_maleficent.dxf");
}

module meeple_maleficent_cookie_cutter(cutter_scale, cutter_height, cutter_thickness, cutter_color, filament="PLA"){
  difference() {
    color(cutter_color) cutterize_3d_offset(thickness = cutter_thickness, height =cutter_height) scale(cutter_scale) meeple_maleficent_shape();
    union () {
...
module meeple_maleficent_mold(mold_height, mold_radius, mold_fillet_steps, mold_color){
  color(mold_color) molderize_n_fillet(height =mold_height, radius=mold_radius, steps=mold_fillet_steps) meeple_maleficent_mold_shape (); 
}
